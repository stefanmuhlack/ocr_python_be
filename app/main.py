...
async def process_ocr(file: UploadFile = File(...), user: dict = Depends(get_current_user)):
    try:
        ...
        logger.info(f"OCR processed successfully for user {user['username']}")
        return JSONResponse(status_code=200, content={"message": "OCR processed successfully!", "results": results})
    } except (Exception as e) {
        logger.error(f"Failed to process OCR for user {user['username']}: {str(e)}")
...
async def secure_endpoint(token: str = Depends(oauth2_scheme)) {
    ...
    logger.info("Accessed secure content")
